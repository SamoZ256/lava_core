...
void OpenGL_DescriptorSet::bind() {
    ...
    for (uint32_t i = 0; i < imageInfos.size(); i++) {
        ...
        std::cout << "Image binding: " << g_opengl_swapChain->activeGraphicsPipeline->bindings[layoutIndex][imageBindingIndices[i]] << " : " << textureBinding << std::endl;
        ...
        std::cout << "Sampler: " << imageInfos[i].sampler << std::endl;
    }
